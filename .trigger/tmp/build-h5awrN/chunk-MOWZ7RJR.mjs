...
var require_paeth_predictor = __commonJS({
  "node_modules/pngjs/lib/paeth-predictor.js"(exports, module) {
    "use strict";
    init_esm();
    module.exports = /* @__PURE__ */ __name(function paethPredictor(left, above, upLeft) {
...
function cropZone(png, zone) {
  ...
  const out = new import_pngjs.PNG({ width: srcW, height: srcH });
  import_pngjs.PNG.bitblt(png, out, srcX, srcY, srcW, srcH, 0, 0);
  return import_pngjs.PNG.sync.write(out);
}
__name(cropZone, "cropZone");
var BLOCKED_DOMAINS = [
  "doubleclick.net",
...
async function preparePageForScreenshot(page) {
  ...
  await page.evaluate(() => {
    ...
    for (const el of document.querySelectorAll(`${CONTAINERS} button, ${CONTAINERS} a[role="button"]`)) {
    ...
    for (const sel of REMOVE_SELECTORS) {
      try {
        document.querySelectorAll(sel).forEach((el) => {
          ...
          if (style.position === "fixed" || style.position === "sticky" || el.getAttribute("role") === "dialog" || el.getAttribute("aria-modal") === "true") {
            el.remove();
          }
        });
      } catch {
...
__name(preparePageForScreenshot, "preparePageForScreenshot");
async function processUrl(monUrl, supabase, now = /* @__PURE__ */ new Date()) {
  ...
  try {
    ...
    await page.goto(monUrl.url, { waitUntil: "networkidle", timeout: 3e4 });
  ...
  const { error: uploadErr } = await supabase.storage.from("screenshots").upload(screenshotPath, screenshotBuffer, { contentType: "image/png", upsert: false });
  if (uploadErr) {
    logger.error("Upload failed", { url: monUrl.url, error: uploadErr.message });
    throw new Error(`Upload failed: ${uploadErr.message}`);
  }
  const { data: snapshot, error: snapErr } = await supabase.from("screenshot_snapshots").insert({
  ...
  const { data: prevSnapshot } = await supabase.from("screenshot_snapshots").select("*").eq("monitored_url_id", monUrl.id).neq("id", snapshot.id).order("taken_at", { ascending: false }).limit(1).single();
  await supabase.from("monitored_urls").update({ last_checked_at: now.toISOString() }).eq("id", monUrl.id);
  if (!prevSnapshot) {
  ...
  let diffPct = 0;
  let diffStoragePath = null;
  let prevBuffer = null;
  let zoneCrops;
  const zones = Array.isArray(monUrl.zones) ? monUrl.zones : [];
  try {
    prevBuffer = Buffer.from(await prevBlob.arrayBuffer());
    const prevPng = import_pngjs.PNG.sync.read(prevBuffer);
    const currPng = import_pngjs.PNG.sync.read(screenshotBuffer);
    if (zones.length > 0) {
      zoneCrops = [];
      ...
      if (zoneCrops.length === 0) zoneCrops = void 0;
    }
    ...
    const diffImg = new import_pngjs.PNG({ width, height: minH });
    const changedPixels = (0, import_pixelmatch.default)(prevSlice, currSlice, diffImg.data, width, minH, { threshold: 0.1 });
    const extraPixels = width * (maxH - minH);
    diffPct = (changedPixels + extraPixels) / totalPixels * 100;
    if (diffPct > 0) {
      const diffBuffer = import_pngjs.PNG.sync.write(diffImg);
  ...
  if (diffPct >= AI_FLOOR) {
    ...
    if (prevBuffer) {
      try {
        const aiResult = await analyzeWithAI({
          ...
          thresholdPct: monUrl.threshold_pct ?? void 0
        });
    ...
    if (shouldAlert) {
      const severity = diffPct >= 50 ? "critical" : diffPct >= 25 ? "high" : diffPct >= 10 ? "medium" : "low";
...
//# sourceMappingURL=chunk-MOWZ7RJR.mjs.map
