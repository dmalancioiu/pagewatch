...
// trigger/lib/analyze-diff-with-ai.ts
init_esm();
...
// trigger/lib/analyze-diff-with-ai.ts
var NO_ALERT_MARKER = "NO_ALERT";
async function analyzeWithAI(params) {
  const { beforeBuffer, afterBuffer, diffPct, watchDescription, thresholdPct } = params;
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    return {
      shouldAlert: true,
      summary: genericSummary(diffPct)
    };
  }
  const client = new Anthropic({ apiKey });
  const beforeB64 = beforeBuffer.toString("base64");
  const afterB64 = afterBuffer.toString("base64");
  let prompt;
  const sensitivityNote = thresholdPct != null ? `The user's sensitivity is set to ${thresholdPct}% — changes smaller than this are generally considered noise by them.` : "";
...
__name(analyzeWithAI, "analyzeWithAI");
function genericSummary(diffPct) {
...
__name(genericSummary, "genericSummary");

export {
  analyzeWithAI
};
//# sourceMappingURL=chunk-NP4EJFZK.mjs.map
