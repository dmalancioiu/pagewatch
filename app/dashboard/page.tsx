          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.35fr) minmax(300px, 0.85fr)', gap: 18, alignItems: 'start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <Surface>
                ...
                <div style={{ padding: '16px 18px 18px' }}><div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 18 }}><div style={{ display: 'grid', gridTemplateRows: 'repeat(7, 14px)', gridAutoFlow: 'column', gridAutoColumns: '14px', gap: 5 }}>{heatmapDays.map(day => <div key={day.key} title={`${day.label}: ${day.count} change${day.count === 1 ? '' : 's'}`} style={{ width: 14, height: 14, borderRadius: 4, background: heatColor(day.count), border: '1px solid rgba(15,23,42,0.04)' }} />)}</div><div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 1, flexShrink: 0 }}><span style={{ fontSize: 10, color: '#94A3B8' }}>Fewer</span><div style={{ display: 'flex', gap: 4 }}>{[0, 1, 3, 5].map(value => <span key={value} style={{ width: 13, height: 13, borderRadius: 4, background: heatColor(value), border: '1px solid rgba(15,23,42,0.04)' }} />)}</div><span style={{ fontSize: 10, color: '#94A3B8' }}>More</span></div></div></div>
              </Surface>
            </div>
