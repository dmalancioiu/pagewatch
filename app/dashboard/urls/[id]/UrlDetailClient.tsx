...
export function UrlDetailClient({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {activeAlert && (
        <div style={{
          background: 'linear-gradient(90deg, rgba(254,242,242,0.92), rgba(255,255,255,0.92))',
          ...
          <div style={{ width: 28, height: 28, borderRadius: 9, background: 'rgba(220,38,38,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
            <button className="btn-dash-ghost" onClick={() => startTransition(async () => { await acknowledgeAlert(activeAlert.id); setDismissed(true) })} disabled={isPending} style={{ fontSize: 11 }}>
              <CheckCircle2 size={10} /> {isPending ? 'Saving…' : 'Resolve'}
            </button>
            <button className="btn-dash-primary" onClick={() => openModal(activeAlert, null, 'compare')} style={{ padding: '5px 11px', fontSize: 11 }}>
              <Maximize2 size={10} /> View Diff
            </button>
          </div>
        </div>
      )}
      ...
      {latestSnap ? (
        <div style={{
          ...
          borderRadius: 16,
          overflow: 'hidden',
          boxShadow: '0 18px 48px rgba(15,23,42,0.06), 0 1px 2px rgba(15,23,42,0.04)',
        }}>
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
              <div style={{ width: 34, height: 34, borderRadius: 11, background: 'rgba(37,99,235,0.08)', border: '1px solid rgba(37,99,235,0.16)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 11, color: '#94A3B8', marginTop: 3 }}>
                  {zones.length > 0 ? `${zones.length} focused region${zones.length !== 1 ? 's' : ''}. Only meaningful changes inside these areas should alert you.` : 'No focus zones yet. Start by selecting the parts of the page that matter.'}
                </p>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 7 }}>
              <button className="btn-dash-primary" onClick={() => openModal(null, latestSnap, 'after')} style={{ padding: '7px 13px', fontSize: 12, borderRadius: 9, boxShadow: '0 8px 18px rgba(37,99,235,0.18)' }}>
                <Sparkles size={12} /> Edit zones
              </button>
              <button className="btn-dash-ghost" onClick={() => openModal(openAlert, latestSnap, openAlert ? 'compare' : 'after')} style={{ padding: '7px 12px', fontSize: 12, borderRadius: 9 }}>
                <Maximize2 size={11} /> Capture
              </button>
            </div>
          </div>
            ...
            <div style={{ padding: 18 }}>
              {zones.length > 0 ? (
                  ...
                  {zones.map((z, i) => {
                    ...
                    return (
                      <div key={z.id} style={{
                        ...
                        borderRadius: 13,
                        background: changed
                          ? 'linear-gradient(90deg, rgba(255,255,255,1), rgba(254,242,242,0.56))'
                          : 'linear-gradient(90deg, #FFFFFF, #FBFDFF)',
                        ...
                        <div style={{ minWidth: 0 }}>
                          <p style={{ fontSize: 11, color: '#64748B', lineHeight: 1.45, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {z.instruction?.trim() || `Alert if ${z.label || `Zone ${i + 1}`} changes.`}
                          </p>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginTop: 7 }}>
                            <span style={{ fontSize: 10, color: '#94A3B8' }}>Latest diff</span>
                            <span style={{ fontSize: 11, fontWeight: 800, color: changed ? '#DC2626' : '#0F172A' }}>{latestAlert?.diff_pct != null ? `${Number(latestAlert.diff_pct).toFixed(1)}%` : '—'}</span>
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <span style={{
                            ...
                            borderRadius: 999,
                            padding: '4px 8px',
                            color: changed ? '#B91C1C' : '#15803D',
                            background: changed ? 'rgba(220,38,38,0.07)' : 'rgba(22,163,74,0.08)',
                            border: changed ? '1px solid rgba(220,38,38,0.14)' : '1px solid rgba(22,163,74,0.14)',
                          }}>
                            {changed ? 'Changed' : 'Clean'}
                          </span>
                          <button onClick={() => openModal(openAlert, latestSnap, openAlert ? 'compare' : 'after')} style={{ border: 'none', background: '#F8FAFC', padding: '6px 8px', borderRadius: 8, fontSize: 11, color: '#2563EB', fontWeight: 750, cursor: 'pointer' }}>
                            View
                          </button>
                        </div>
                      </div>
                ...
                <div style={{ border: '1px dashed #CBD5E1', borderRadius: 14, padding: '32px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', background: 'linear-gradient(180deg, #F8FAFC, #FFFFFF)' }}>
                  ...
                  <button className="btn-dash-primary" onClick={() => openModal(null, latestSnap, 'after')} style={{ fontSize: 12, borderRadius: 9 }}><Plus size={11} /> Create focus zones</button>
                </div>
              )}
            </div>
            ...
            <div style={{ borderLeft: '1px solid #F1F5F9', background: 'linear-gradient(180deg, #F8FAFC, #FFFFFF)', padding: 16 }}>
              ...
              <div style={{ marginTop: 13, display: 'flex', flexDirection: 'column', gap: 1, borderTop: '1px solid #E5E7EB', borderBottom: '1px solid #E5E7EB' }}>
                {[
                  ...
                  { label: 'Changes', value: changeEvents, color: changeEvents > 0 ? '#DC2626' : undefined },
                  ...
                  <div key={stat.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '9px 0' }}>
                    <span style={{ fontSize: 10, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 800 }}>{stat.label}</span>
                    <span style={{ fontSize: 14, fontWeight: 850, color: stat.color ?? '#0F172A' }}>{stat.value}</span>
                  </div>
                ))}
              </div>
      ...
      <div style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: 8, overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', borderBottom: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, background: '#FAFAFA' }}>
          ...
          <div style={{ display: 'flex', background: '#F3F4F6', borderRadius: 6, padding: 2, gap: 1 }}>
            {(['all', 'changes', 'clean'] as TFilter[]).map(key => <button key={key} onClick={() => { setFilter(key); setVisibleCount(PAGE_SIZE) }} style={{ padding: '3px 10px', borderRadius: 5, fontSize: 11, fontWeight: 600, background: filter === key ? 'white' : 'transparent', color: filter === key ? '#111827' : '#9CA3AF', border: 'none', cursor: 'pointer', boxShadow: filter === key ? '0 1px 2px rgba(0,0,0,0.08)' : 'none', textTransform: 'capitalize', transition: 'all 0.1s' }}>{key}</button>)}
          </div>
        </div>
        <div>
          {filtered.length === 0 ? <div style={{ padding: '28px 14px', textAlign: 'center', color: '#9CA3AF', fontSize: 12 }}>No snapshots match this filter.</div> : <>{groups.map(({ label, items }) => <div key={label}><div style={{ padding: '8px 14px 3px', fontSize: 10, fontWeight: 700, color: '#D1D5DB', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{label}</div>{items.map(snap => <SnapshotRow key={snap.id} snap={snap} alert={alertBySnapshotId[snap.id]} changed={!!alertBySnapshotId[snap.id]} onOpen={() => openModal(alertBySnapshotId[snap.id] || null, snap, alertBySnapshotId[snap.id] ? 'compare' : 'after')} onDownload={() => snap.signedUrl && downloadImage(snap.signedUrl, `snap-${snap.id}.png`)} />)}</div>)}<div style={{ padding: '12px 14px', textAlign: 'center' }}>{hasMore ? <button className="btn-dash-ghost" onClick={() => setVisibleCount(c => c + PAGE_SIZE)} style={{ fontSize: 11 }}>Show more · {filtered.length - visibleCount} remaining</button> : <p style={{ fontSize: 10, color: '#D1D5DB', letterSpacing: '0.06em', textTransform: 'uppercase' }}>End of History</p>}</div></>}
        </div>
      </div>
